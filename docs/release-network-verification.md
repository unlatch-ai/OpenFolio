...
1. Record the candidate and environment:
   ...
   ```bash
   shasum -a 256 OpenFolio-0.4.4-arm64.dmg OpenFolio-0.4.4-arm64.zip | tee artifact-sha256.txt
...
2. Start Apple `pktap` capture before launching the app. `-k NP` preserves
   ...
   ```bash
   sudo tcpdump -i pktap,all -n -U -k NP -w openfolio-v0.4.4.pcap
   ```
...
```bash
tcpdump -nn -r openfolio-v0.4.4.pcap -k NP > openfolio-packets.txt
rg -n -i 'OpenFolio|openfolio|chrome_crashpad_handler|ShipIt' openfolio-processes-and-sockets.txt
shasum -a 256 openfolio-v0.4.4.pcap openfolio-nettop.csv openfolio-processes-and-sockets.txt artifact-sha256.txt environment.txt codesign.txt stapler.txt gatekeeper.txt > evidence-sha256.txt
```
