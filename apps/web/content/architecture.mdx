...
Hosted services are planned future layers for:
...
The current MCP path is local stdio against the local graph. Future hosted MCP may rely on an optional derived hosted graph, but raw Messages remain local by default.
