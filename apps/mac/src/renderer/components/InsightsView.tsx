...
function Strata({ data }: { data: WrappedSummary["messagesByMonth"] }) {
  ...
  return (
    <div
      className="wrapped-strata"
      role="img"
      aria-label={`Monthly message counts: ${data.map((entry) => `${monthName(entry.month)} ${entry.count}`).join(", ")}`}
    >
      {data.map((entry) => (
        <span
          key={entry.month}
          style={{ height: `${Math.max(3, (entry.count / max) * 100)}%` }}
        >
          <i>{entry.count.toLocaleString()}</i>
        </span>
      ))}
    </div>
  );
...
function Heatmap({ data }: { data: MessageHeatmapEntry[] }) {
  ...
  return (
    <section className="wrapped-module">
      ...
      <h2>{activeDays.length.toLocaleString()} days with messages</h2>
      <div
        className="archive-heatmap"
        role="img"
        aria-label={`${activeDays.length} active days; busiest recorded day had ${max} messages`}
      >
        {data.map((entry) => (
          <span
            key={entry.date}
            ...
            style={
              { "--heat": String(entry.count / max) } as React.CSSProperties
            }
          />
        ))}
      </div>
        ...
        <ul>
          {activeDays.map((entry) => (
            <li key={entry.date}>
              ...
              {entry.count.toLocaleString()} messages
            </li>
          ))}
        </ul>
      </details>
...
export function InsightsView() {
  ...
  const load = useCallback(async (value: number) => {
    ...
    try {
      const [summary, days] = await Promise.all([
        ...
        window.openfolio.insights.getMessageHeatmap(value),
      ]);
      setWrapped(summary);
      setHeatmap(days);
    } finally {
  ...
  const busiestDay = useMemo(
    () =>
      wrapped
        ? wrapped.messagesByDayOfWeek.reduce(
    ...
    return (
      <main className="wrapped-view">
        <header className="wrapped-header">
          <Skeleton className="wrapped-title-skeleton" />
        </header>
      ...
      <main className="wrapped-view">
        <header className="wrapped-header">
          ...
          <div className="year-nav">
            ...
            <Button
              variant="ghost"
              onClick={() => setYear(Math.min(now, year + 1))}
              disabled={year === now}
  ...
  return (
    <main className="wrapped-view">
      ...
      <article className="wrapped-artifact">
        <section className="wrapped-opening" data-year={year}>
          <p>
            {wrapped.totalMessages.toLocaleString()} messages across{" "}
            {wrapped.totalConversations.toLocaleString()} conversations
          </p>
          <h2>
        ...
        <div className="wrapped-grid">
          <section className="wrapped-module">
            ...
            <dl className="wrapped-totals">
              <div>
                <dt>Messages</dt>
                <dd>{wrapped.totalMessages.toLocaleString()}</dd>
              </div>
              <div>
                <dt>Conversations</dt>
                <dd>{wrapped.totalConversations.toLocaleString()}</dd>
              </div>
              <div>
                <dt>Daily average</dt>
                <dd>{wrapped.avgDailyMessages.toLocaleString()}</dd>
              </div>
            </dl>
          </section>
          <section className="wrapped-module">
            <p className="eyebrow">Daily rhythm</p>
            <h2>{DAYS[busiestDay]} was the busiest weekday.</h2>
            <ul className="rhythm-list">
              {wrapped.messagesByDayOfWeek.map((count, index) => (
                <li key={DAYS[index]}>
                  <span>{DAYS[index]}</span>
                  <i
                    style={{
                      width: `${Math.max(2, (count / Math.max(...wrapped.messagesByDayOfWeek, 1)) * 100)}%`,
                    }}
                  />
          ...
          <section className="wrapped-module top-people">
            <p className="eyebrow">Top people</p>
            {wrapped.topContacts.map((person, index) => (
              <button
                key={person.personId}
                onClick={() => {
              ...
              >
                ...
                <i style={{ background: personColor(person.personId) }} />
                <strong>{person.displayName}</strong>
                <em>{person.totalMessages.toLocaleString()}</em>
              </button>
            ))}
          </section>
